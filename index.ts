...
const retrieveSessionID = async (groupID: string) => {
    let res: OkPacket | Array<any> = await dbQuery(
...
// USER
fastify.post<{ Body: { emailAddress: string; password: string } }>(
    "/api/user/login",
    async (request, reply) => {
        const { body } = request;
        ...
        if (await argon2.verify(results[0].password, body["password"])) {
            ...
            if (!results[0].authenticationKey) {
                await dbInsert(
                    "UPDATE users SET authenticationKey=? WHERE emailAddress=?",
                    [code, body["emailAddress"]]
...
fastify.post<{ Body: { authenticationKey: string } }>(
    ...
    async (request, reply) => {
        ...
        const results = await dbQuery(
            "SELECT emailAddress FROM users WHERE userID=?",
            [await retrieveID(body["authenticationKey"])]
        );
        if (!results.length)
            return reply.code(400).send("There is no user with that ID");
        await dbQuery(
            "UPDATE users SET active=0, verificationCode=? WHERE authenticationKey=?",
            [emailCode, body["authenticationKey"]]
        );
...
fastify.post<{ Body: { authenticationKey: string } }>(
    ...
    async (request, reply) => {
        ...
        const res = await dbQuery("UPDATE groups SET premium=1 WHERE groupID=?", [
            groupID,
        ]);
...
fastify.post<{ Body: { authenticationKey: string; groupID: string } }>(
    ...
    async (request, reply) => {
        ...
        const results = await dbQuery(
            "SELECT groupID, premium FROM groups WHERE groupID=?",
            [groupID]
        );

        if (!results.length)
            return reply.code(400).send("There was no group found with that ID!");

        const isPremium = (
            await dbQuery("SELECT premium FROM groups WHERE groupID=?", [
                await retrieveGroupID(body["authenticationKey"]),
            ])
        )[0]?.premium;
        const isNewPremium = results[0]?.premium;
        const groupMemberCount = await dbQuery(
            "SELECT null FROM users WHERE groupID=?",
            [groupID]
        );
        const lastInGroup = await checkIfLast(body["authenticationKey"]);
...
fastify.post<{ Body: { authenticationKey: string; newEmail: string } }>(
    ...
    async (request, reply) => {
        ...
        if (!results.length)
            return reply.code(400).send("There is no user with that ID");
        await dbQuery(
            "UPDATE users SET verificationCode=?, tempEmail=? WHERE authenticationKey=?",
            [emailCode, body["newEmail"], body["authenticationKey"]]
        );
...
fastify.post<{ Body: { emailAddress: string } }>(
    ...
    async (request, reply) => {
        ...
        if (!results.length)
            return reply.code(400).send("There is no user with that email address");
        await dbQuery("UPDATE users SET verificationCode=? WHERE emailAddress=?", [
            emailCode,
            body["emailAddress"],
        ]);
...
fastify.post<{ Body: { authenticationKey: string; newPassword: string } }>(
    ...
    async (request, reply) => {
        ...
        await dbQuery(
            "UPDATE users SET password=?, authenticationKey=null WHERE authenticationKey=?",
            [password, body["authenticationKey"]]
        );
    }
);
    ...
    async (request, reply) => {
        ...
        if (!results[0]["active"])
            return reply
                .code(400)
                .send(
                    "This account has been deactivated. Please check your emails to reactivate!"
                );
...
fastify.get<{ Querystring: { authenticationKey: string } }>(
    ...
    async (request, reply) => {
        ...
        const results = await dbQuery(
            "SELECT fullName, groupID FROM users WHERE userID=?",
            [userID]
        );
...
fastify.post<{
    ...
    const results = await dbQuery(
        "SELECT l.distance, s.sessionActive, s.initialOdometer, s.sessionID, u.fullName, u.notificationKey, u.userID FROM logs l LEFT JOIN sessions s USING (sessionID) LEFT JOIN users u ON l.userID = u.userID WHERE s.groupID=? AND s.sessionActive=1",
        [await retrieveGroupID(body["authenticationKey"])]
    );
