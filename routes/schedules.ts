...
export default (fastify: FastifyInstance, _: any, done: () => void) => {
  fastify.post<{
    Body: {
      ...
      summary: string;
      repeating: string;
      custom: { number: string; repeatingFormat: string; repeatingDays: string[] };
    ...
    if (!body || !("startDate" in body) || !("allDay" in body) || !("endDate" in body) || !userData) {
    ...
    if (!body.allDay && endDate.getTime() <= endTimeInterval.getTime()) {
      return reply
        .code(400)
        .send("Please choose a valid end date combination more then 30 minutes after your start time!");
    }
    ...
    if (isUnique.length === 0) {
      if (body.scheduleID) {
        await dbQuery("UPDATE schedules SET allDay=?, startDate=?, endDate=?, summary=? WHERE scheduleID=?", [
          body.allDay,
          startDate,
          endDate,
          body.summary,
        ...
        const { insertId } = await dbInsert(
          "INSERT INTO schedules(allDay, startDate, endDate, summary, groupID, userID) VALUES (?,?,?,?,?,?)",
          [body.allDay, startDate, endDate, body.summary, groupID, userID]
        );
        linkedID = insertId;
      }
    ...
    while (count !== limit) {
      ...
      count++;
      if (
        body.repeating === "custom" &&
        body.custom.repeatingDays.length &&
        body.custom.repeatingFormat === "weekly" &&
        !body.custom.repeatingDays.filter((day) => parseInt(day) === start.getDay()).length
      )
        continue;
      ...
      if (isUnique.length === 0) {
        dbInsert(
          "INSERT INTO schedules(allDay, startDate, endDate, summary, groupID, userID, linkedSessionID) VALUES (?,?,?,?,?,?,?)",
          [body.allDay, start, end, body.summary, groupID, userID, linkedID]
        );
  ...
  fastify.get<{
    ...
    const data: { startDate: Date; endDate: Date; allDay: boolean; summary?: string; userID: string }[] = await dbQuery(
      "SELECT startDate, endDate, allDay, summary, userID, scheduleID, linkedSessionID FROM schedules WHERE groupID=?",
      [groupID]
    );
...
const checkForDuplicates = async (groupID: string, startDate: Date, endDate: Date, scheduleID?: string) => {
  const dates = await dbQuery("SELECT startDate, endDate, userID, scheduleID FROM schedules WHERE groupID=?", [
    groupID,
  ]);
