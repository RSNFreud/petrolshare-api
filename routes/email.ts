...
export default (fastify: FastifyInstance, _: any, done: () => void) => {
  fastify.get<{ Querystring: { authenticationKey: string; code: string } }>("/email/verify", async (request, reply) => {
    ...
    const results = await dbQuery("SELECT fullName, verified, tempEmail FROM users WHERE verificationCode=?", [
      query["code"],
    ]);
    ...
    else await dbInsert("UPDATE users SET verified=1, verificationCode=null WHERE verificationCode=?", [query["code"]]);
  ...
  fastify.get<{ Querystring: { code: string } }>("/email/deactivate", async (request, reply) => {
    ...
    await dbInsert("UPDATE users SET active=0, verificationCode=? WHERE verificationCode=?", [
      verificationCode,
      query["code"],
    ]);
    sendMail(
