...
import { retrieveGroupID, dbQuery, checkIfLast, generateEmailCode, retrieveID, sendMail, dbInsert, generateCode, UserTableType } from "../hooks";
...
export default (fastify: FastifyInstance, _: any, done: () => void) => {
    fastify.post<{ Body: { emailAddress: string; password: string } }>(
        ...
        async (request, reply) => {
            ...
            const results: UserTableType[] = await dbQuery(
